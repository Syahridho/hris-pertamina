...
<style>
    ...
    .placement-info svg { width: 20px; height: 20px; color: var(--red); flex-shrink: 0; }
    ...
    .btn-refresh   { background: var(--navy-card); border: 1px solid var(--border); color: var(--text-muted); padding: 12px 16px; }
...
<div class="placement-info">
    ...
    <div>
        <div style="font-weight:600">{{ $placement?->name ?? 'Lokasi belum dikonfigurasi' }}</div>
        <div style="font-size:12px;color:var(--text-muted)">Radius: {{ $placement?->radius ?? 0 }} meter</div>
    </div>
</div>
...
<div class="attendance-record">
    ...
    <div class="record-row">
        <span class="record-label">Durasi</span>
    ...
    <div class="record-row">
        ...
        <span class="record-value" style="color:{{ $todayAttendance->status === 'late' ? '#f59e0b' : '#10b981' }}">
...
<script>
        ...
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                if (distance === null) {
                    ...
                    subText.textContent = 'Hubungi supervisor atau admin.';
                    return;
                }
                ...
                if (distance <= PLACEMENT_RADIUS) {
                    ...
                    subText.textContent = 'Absensi dapat dilakukan sekarang.';
                    badge.className = 'distance-badge distance-ok';
                    badge.textContent = `${Math.round(distance)}m dari lokasi kerja`;
                    ...
                    subText.textContent = `Anda perlu lebih dekat ke lokasi kerja.`;
                    badge.className = 'distance-badge distance-far';
                    badge.textContent = `${Math.round(distance)}m — Radius maks: ${PLACEMENT_RADIUS}m`;
                    badge.style.display = 'inline-block';
            ...
            (err) => {
                ring.className = 'ring-outer invalid';
                icon.innerHTML = SVG_CROSS;
                statusText.textContent = 'Izin lokasi ditolak';
    ...
    async function doClock(type) {
        ...
        const url = type === 'in' ? '{{ route("manpower.clock.in") }}' : '{{ route("manpower.clock.out") }}';
